...
function isShape(kind: ArenaPatternKind): boolean {
 ...
 */
export function pickArenaPattern(rand: Rand, slots: number): ArenaPattern {
  const kind = ARENA_PATTERN_KINDS[Math.trunc(rand.nextFloat() * ARENA_PATTERN_KINDS.length)]
  ...
  // Rolled unconditionally for a shape and never for a field pattern, so the
  // draw count is fixed per kind.
  let inside = 0
  let outside = 0
  if (isShape(kind)) {
    inside = 1 + Math.trunc(rand.nextFloat() * (slots - 1))
    outside = rand.nextFloat() < 0.75 ? 0 : (inside % (slots - 1)) + 1
  }

  return { kind, scale, inside, outside, slots }
}
