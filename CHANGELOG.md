...
- **Free upgrade pickups in the lobby and the boss prep room.** Both hand-authored rooms can lay the game's eight stock upgrade items on the floor — damage, defense, health, mana and their tier-2 versions — as a per-room, per-kind count with no gameplay cap. Each kind has a single authored slot, so anything above one stacks on that slot rather than needing the room's layout to grow, and a kind left at zero lays out nothing at all. Every kind defaults to **0**, so a stock run's balance is unchanged and the **Free upgrades** section starts collapsed on both the Lobby tab and Boss → Prep. Both rooms also gained two extra warm lights, taken from levels saved out of the game's own editor; they are unconditional and have no parameter. Neither addition draws from any RNG stream — every `levels/level*.xml` stays byte-identical however many upgrades a room hands out. In `parameters.txt` this is `lobbyUpgrades` / `bossUpgrades`, eight space-separated counts; a missing, short or malformed value is reported, never fatal.
- **Buffs per boss wave tier.** The arena's five tiers — 100%, 75%, 50%, 25% and after the boss dies — can each carry an arena-wide buff aimed at players, monsters or both. Unlike the wave *spawns*, which switch on and never off, the buffs **replace** one another: each threshold switches the previous field off as it switches its own on, so exactly one aura is ever live and the fight reads as phases rather than as a growing pile of debuffs. The 100% buff is live from the moment the fight starts. Because copying the previous tier is the common setup — an aura that should cover the whole second half has to be repeated on each tier that would otherwise clear it — every tier has a **Copy to tiers below** button. A new **Wave buffs** section sits directly under **Waves** in the Boss tab; no tier carries one by default, generated dungeon floors are byte-identical either way, and so is the arena itself for the same seed. In `parameters.txt` this is `bossWaveBuffN=<id>:<target>` — its own key rather than a sixth field on `bossWaveN`, so files exported before the feature still round-trip byte-for-byte.
- **Buff auras per floor.** Hang any of the game's 41 buffs on any dungeon floor and choose who it catches — **players**, **monsters**, or **both**. A buff aura covers the whole floor and is live from the moment the party arrives; there is no countdown and it never switches off, so it reads as a property of the floor rather than an event on it. Slow the party with `frost`, enrage the horde with `bloodlust`, poison everything with `slime_poison`, or drain the party's mana the whole way down. A floor may carry several at once, each with its own target, so the same floor can slow the party *and* hurry the monsters. Every buff in the dropdown carries a description of exactly what it does — duration, damage, speed and damage multipliers — on hover and beside the row, so you never need to open the game's own asset folder. A new **Buffs per floor** section sits between **Monster max counts** and **Timer mode** in the Dungeon tab, with the same **Copy to floors below** button; no floor carries a buff by default, and a floor left empty produces byte-identical XML to the pre-feature generator for the same seed. In `parameters.txt` this is `buffN=<id>:<target>|<id>:<target>`, written only for floors that carry one.
...
- **The final room opens with a button, not a gold key.** `finalLockMode` (default `button`) bars the orb's corridor with a destructible wall and hides a floor button elsewhere on the floor, placed the way a key is: stepping on it plays the hatch sound and blows the wall open. No key is involved, so a party that hoarded gold keys from earlier floors — or spent this floor's on a vault — can still finish the campaign. `finalLockMode=key` keeps the original gold door, now with one gold key per gold door on that floor.
