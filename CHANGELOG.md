...
- **Rearrangeable lobbies.** The lobby and the boss prep room were the same room wearing two names — the same shop stalls, the same diamond payout, the same free upgrades, the same revive-on-arrival, built by two copies of one function. They are now one thing, a **lobby**, with a **preset** picking which hand-authored room it is: `BETA-dungeon-prep` (what used to be the lobby) or `BETA-boss-prep` (what used to be the prep room). A campaign can hold **as many as you like**, each fully independent — its own preset, gold, shop columns and free upgrades — and the Floor order tab places them anywhere as `L1 … Ln`: one at the front as a starting hub, one before each fight, one halfway through the floors, none at all. The Lobby tab gains a count field, `Lobby 1 / Lobby 2 …` sub-tabs and a copy button, mirroring the fight tabs; the Boss tab loses its Prep room sub-tab, because a fight is an arena now. A lobby may not be the campaign's **last** slot — it carries no victory orb — and the reorder arrow is disabled rather than reporting it afterwards. A floor now has **three** visually distinct ways out, decided by what the order puts next: stairs to another floor, the red portal into an arena, and a new **blue teleport** into a lobby. `parameters.txt`: `lobbies=2`, `lobby<i>Preset|Gold|Shops|Upgrades`, and `levelOrder` accepts `L` tokens (`levelOrder=L1,1,2,L2,B1,3`).

  **Breaking**, deliberately, pre-release: `lobby`, `lobbyGold`, `lobbyShops`, `lobbyUpgrades` and `boss<i>Gold` / `boss<i>Shops` / `boss<i>Upgrades` are gone. A file carrying them still imports — they are reported as unknown keys, never fatal — but their values are ignored, so re-enter that room's settings as a lobby. The stock campaign emits the same rooms it did before (`L1 1…7 L2 B1 8`), and because the blue portal shares the red one's room-selection branch, every existing seed's escape floor is byte-identical; the fight-adjacent floor differs only in its portal art and target. A boss fight is entered at its arena rather than its prep room, so `bossprep<i>.xml` no longer exists and floors that lead into a fight name `boss<i>` instead.

- **Traps — projectile spewers on the boss arena walls.** A new Boss-tab section under Wave pickups. Each of the five health tiers can run any number of spewers: pick a projectile from the 45 of the game's own that work as a hazard, the direction it fires, how wide it fans (spread, 0 to 2 in decimals), how often it shoots in milliseconds, and how many to place. A trap stands on the wall it fires *away* from — `up` on the south wall shooting north, `down` on the north, `left` on the east, `right` on the west — on the innermost floor tile, at a seeded position clear of the corners, the entrance, the alcove mouth, the cover pillars and each other. Several rows can share a direction, so one wall can mix ammunition: three axes at 100 ms with 0.5 spread plus two homing fireballs at 1500 ms with none. Like the wave buffs and unlike the drops, a tier's traps **replace** the previous tier's, so the arena's hazard changes with the phase instead of piling up into a crossfire nobody can cross; the 100% tier is live from the moment the fight loads. Every projectile's tooltip carries its real damage, speed and behaviour read off the game's own files. The roster is deliberately 45 of the game's 68: a projectile with `damage: 0` gets its damage from the weapon and the character firing it, and a spewer has neither, so all 22 of those (every `player_*`, the sorcerer shards, the coin spray) are decoration from a wall and are not offered, and `sorcerer_ice_orb` — the Sorcerer group's only other entry — crashes the game outright when a spewer fires it. Two things the tooltips do still warn about: `shooter_stone_ball` does 1000 damage, an unconditional kill, and a `seeker` fired from a wall does **not** home — behaviour belongs to the monster's aim, not the shot. `parameters.txt`: `boss<i>WaveTrapN=<projectile>:<direction>:<spread>:<rate>:<count>|…`, on its own key, so files written before the feature round-trip byte for byte. This ships **off** — no preset carries a trap — so every existing seed is unchanged. Turning it on shifts that arena's floor-tile cosmetics and any later fight's stream, but never the arena's layout: the same seed lays out the same room with the same pillars, trapped or not.

- **Rearrangeable floors.** A new **Floor order** tab, between Boss and Player, shows the campaign as a row of chips — `1 2 B1 3 4 B2` — that the ◀ ▶ buttons move. A boss fight can sit before the first floor, between two floors, or at the end. Floors keep their relative order and so do fights; only the interleave is free, and the arrows are disabled for an illegal move rather than reporting it afterwards. Whichever slot is **last** holds the victory orb, so a run can end on a dungeon floor instead of in an arena. A mid-campaign floor that leads into a fight gets the boss portal. Every other link follows the order too: the lobby's teleport, the stairs' target, each arena's exit, `levels.xml`, the in-game floor numbering, and the preview tabs (which now read `B1 1 2 B2 3`). `parameters.txt`: `levelOrder=1,2,B1,3`, written only when the order was changed. Absent means the historical order and generates byte-identical output.

- **Save game in lobbies.** A new `lobbySaves` toggle places a save checkpoint (`items/trigger_button_save.xml`) by every lobby's exit, so quitting mid-run doesn't throw the whole campaign away. Campaign-wide rather than per-lobby — a run where only some shop rooms can be saved at is a trap, not a choice. Draws no randomness and touches no `levels/level*.xml` — only the lobby files gain the one item. On by default. `parameters.txt`: `lobbySaves=1`.

- **Boss arena checkpoints.** A new Checkpoints / Save game section on the Boss tab adds two independent milestone presets: **Respawn player** pulls dead or lagging players back into the arena, and **Save game** moves the respawn point and writes a save. Both share four presets — `never`, `50`, `75-50-25`, and `75-50-25-dead` (adds the `Boss Died` event) — and a milestone picked by both gets one shared `GlobalEventTrigger`, never two. Both at "Never" emits nothing at all — no trigger, no Checkpoint node, no RespawnPlayers node — so a disabled feature cannot append so much as an id. Independent of `invulnerability` and the wave rig: it listens to the same `Boss 75%`/`50%`/`25%`/`Boss Died` events but wires its own dedicated triggers, so neither rig's `connections` array is touched. Draws no randomness from any stream. Defaults: respawn at 75/50/25/dead, save at 50%. `parameters.txt`: `boss<i>Checkpoints=<respawnPreset>,<saveGamePreset>`.

- **Music picker.** Swap a dungeon floor, lobby, or boss arena's music for any of the game's stock tracks — the base soundbank's `none`, `main`, `act1`–`act4`, `bonus_1`, `bonus_2`, `boss_1`, `boss_killed`, `boss_final`, `custom_1`, `custom_2`, and the desert bank's `desert_cavern`, `desert_temple`, `desert_village`. A new Music section in the Settings tab holds all three pickers. Written only for levels that chose a track — the default (`default`) emits no `PlayMusic` node at all and leaves the template's own music untouched, so every file written before music existed round-trips byte-for-byte. `parameters.txt`: `musicN=<track>`, `lobby<i>Music=<track>`, `boss<i>Music=<track>`.

- **Wall traps on regular dungeon floors.** Projectile spewers that previously lived only in the boss arena can now line ordinary dungeon floors, live from the moment the floor loads — no health tiers, no triggers, every spewer enabled at spawn. One flat slot pool per direction per floor, spread over every eligible room in index order so bigger rooms attract proportionally more traps; the entrance, shop and sealed final room are skipped whole, but a locked vault is deliberately kept. Shares the arena's placement primitives (direction enum, tile-centre correction, wall margin and spacing) extracted to `traps/slots.ts` — the arena's suite passes untouched, which is the proof the extraction moved no seed. RNG is a fourth stream, `ctx.trapRand` (seed + 3), so arming a floor leaves every floor's rooms, walls, doodads, actors, items and pre-existing ids byte-identical. Runs after the retry loop accepts a floor, never inside the `Level` constructor, so it can't tie a floor's traps to how many times reachability rejected it. `count` has no upper bound — `floorTrapCapacity` computes the real ceiling and only warns, never blocks. Off by default; ships empty so the stock campaign is unchanged. `parameters.txt`: `trapN=<projectile>:<direction>:<spread>:<rate>:<count>|…`.
...
- **Campaign level references are now closure-validated.** Every way out of a level — a floor's stairs, an arena's portal, a lobby's teleport — ends up as a `<string name="level">` on a `LevelExitArea` node. A new test walks every such reference against `levels.xml`'s listing and fails if any target is absent, which would otherwise crash the game at runtime with `KeyNotFoundException` in `LevelList.SetLevel`. Every listed level's res file is also checked against the pack's contents, so a dangling target or a phantom entry both fail the build rather than crashing the host.

### Notes

- **Co-op known issues.** When hosting a generated campaign in Hammerwatch 1.41: a rare host crash deep in a boss fight (a Hammerwatch engine issue with networked item pickup, not a fault in the generated campaign — single-player runs clean; reduce wave counts or disable item drops on the later tiers if it recurs), and dead players who reconnect after a crash stay dead for one floor (they revive at the next stairs).
- **Dependency audit.** `npm audit fix` bumps `@xmldom/xmldom` (0.8.13→0.8.15) and `fast-uri` (3.1.5→3.1.7), both dev-only.

## [0.6.0]

### Added

- **Escape floor after the boss, on all three presets.** Every preset now ends on one dungeon floor played *after* the boss arena. Kill the boss, land on the floor, a 90-second clock starts. At zero the floor deals 1 health every 100 ms until you find the orb and leave. The floor is packed with `tower_empty` battlements — roughly a 4-in-9 share of the pool against a cap of 150, a median of 213 per floor. They are 450-HP obstacles that block movement and never attack, so the play is to find the way out, not to clear the floor. Castle gains an 8th floor (`f - mixed`), Desert a 6th (`i - mixed`), Bonus a 6th (`bonus 5`), each with its own chasers, turrets and swarm tiers. This is preset data only, built from `levelOrder`, timer mode and pool weighting; no generator module changed. Two consequences: each preset's old last floor changes shape for a given seed (it was the portal/locked-orb floor and is now an ordinary stairs floor), and a stock `parameters.txt` export gains a `levelOrder=` line and a `timer<N>=` line.

- **Multiple boss fights.** The Boss tab gains a **Number of boss fights** field, a row of **Fight 1, Fight 2, …** tabs above the Prep room / Boss room sub-tabs, and a **Copy to next fight** button. Each fight is fully independent: its own prep room (shop columns, gold, free upgrades) and its own arena (theme, size, boss pool, all five wave tiers with buffs and pickups, cover, spawn tuning, invulnerability, multipliers). Beating a fight teleports the party into the **next** fight's prep room, so a chain reads fight, shop, fight. Only the last arena carries the victory orb and the campaign's single `GameEnd`. There is no upper limit on the count, same as `levels`. The fights share `ctx.bossRand` in list order, so fight 1 is exactly the arena a single-fight campaign has always generated, and every dungeon floor is untouched.

- **Wave pickups — item drops per boss health tier.** A new Boss-tab section under Wave buffs. Each of the five tiers (100/75/50/25% and after the boss dies) can drop any mix of health, mana, potions and free upgrades, with a count per row, up to 64 per row. Copies appear the moment the threshold fires and stay on the floor; unlike the buffs, the tiers do **not** replace one another. They land on a **drop pad** just inside the arena entrance, laid out identically on every seed (health up the left, mana up the right, the eight upgrades in the middle, potions in the row by the door), so a party can learn it once and run back to it mid-fight. Stock table on all three presets: 1× Health (Huge) + 2× Mana (Large) at 50%, one rejuvenation potion at 25%, and double the resupply once the boss is down. `parameters.txt`: `boss<i>WavePickupN=<item>:<count>|…`, on its own key, so files written before the feature still round-trip byte for byte, and a tier the file describes without a pickup line drops nothing rather than inheriting the stock table. This ships **on**, so a stock arena differs from one generated before it. Dungeon floors are unchanged. Extra lives (`powerup_1up`, `powerup_7up`) are in the roster but in no stock table.

### Changed

- **`parameters.txt`: every `boss*` key now carries its fight index.** `bossGold` is `boss0Gold`, `bossWave3` is `boss0Wave3`, and a second fight adds a full `boss1*` block alongside a new `bossFights=N` count. This is a deliberate pre-1.0 break of the flat keys, but importing an older file still works: the unprefixed spelling reads as fight 0. Nothing writes it any more, so re-exporting an old file rewrites it in the indexed form. The emitted campaign follows the same indexing: `levels/bossprep0.xml` and `levels/boss0.xml`, with level ids `bossprep0` / `boss0`.

- **Boss arena rebalanced from two 4-player playtests.** The arena is now 42–64 on both axes, was 24–32 × 32–44. Too small and the horde stacks on itself; too big and a scattered wave arrives dispersed, never re-forms, and gets picked off. Cover defaults to `symmetric` at 0.08 density. Castle's wave counts are cut by about 60% (152/137/117/38/21 per tier) — the 100% tier used to ask for 480 monsters, and roughly 1140 were alive by the 50% threshold. Desert and Bonus keep their tables. `parameters.txt`: `boss<i>Width`, `boss<i>Height`, `boss<i>Cover`.

- **Scattered spawns arrive in batches instead of on one frame.** A monster entry gets at most `batchSize` (8) spawn points per tier; anything past that trickles in every `batchIntervalMs` (1500), the way the anchor rig splits a horde over its nine anchors. Inside the budget the old one-shot shape is emitted unchanged. `parameters.txt`: `boss<i>Spawn` grew two fields to `spacing,ringSpacing,clusters,batchSize,batchIntervalMs`; the three-field form still parses.

- **The horde is bloodlusted after the boss dies.** Every preset's boss-death tier carries `bloodlust` aimed at monsters (+50% damage, +50% move speed), so the walk to the orb is a fight. It is the only tier the stock presets buff. `parameters.txt`: `boss0WaveBuff5=bloodlust:monsters`. This is the first 0.5.0-era feature that ships **on**, so a stock arena is no longer byte-identical to one generated before it. Dungeon floors are unchanged.
...
- **`random` scatter only used the corners and compass points.** Spawn placement accumulated one occupied-floor list across all five tiers, so the floor saturated and every overflow was padded onto the nine anchors. Placement now resets per tier, refuses tiles the pillars sealed off from the entrance, retries a short request at tighter spacing, and pads onto real spare floor before considering an anchor.
