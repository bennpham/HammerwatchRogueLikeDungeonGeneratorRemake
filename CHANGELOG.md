...
- **An escape floor after the boss, on all three presets.** Every shipped campaign now ends on one extra dungeon floor played **after** the boss arena: kill the boss, get spat out onto one more floor, and a **90-second** clock starts — when it runs out the whole floor takes **1 health every 100 ms** until you find the orb and leave. That floor is packed with `tower_empty` battlements (four entries in a nine-entry pool against a cap of 150, so a typical floor carries a couple of hundred of them) walling routes off. They are 450-HP obstacles that block movement but do not attack, so the intended play is to find the way out rather than clear the floor. Castle gets an eighth floor on `f - mixed`, Desert a sixth on `i - mixed`, Bonus a sixth on `bonus 5`, each with its own chasers on top of the battlements. Built entirely out of shipped features — `levelOrder`, timer mode and pool weighting — so no generator behaviour changed. Because the presets now store a non-default `levelOrder`, a stock `parameters.txt` export gains a `levelOrder=` line and one `timer<N>=` line.

- **Rearrangeable floors.** A **Floor order** tab — its own, between Boss and Player — shows the campaign as a row of chips — `1 2 B1 3 4 B2` — that the ◀ ▶ buttons shuffle, so a boss fight can go before the first floor, between two floors, or at the end. Floors keep their own order and so do boss fights; only how the two interleave is free, and the arrows are disabled for a move that would break that rather than reporting it afterwards. Whichever slot is **last** holds the victory orb, so a run can finish on a dungeon floor instead of in an arena — a mid-campaign floor that leads into a fight gets the boss portal, and every other link follows the order too (the lobby's teleport, the stairs' target, each arena's exit, `levels.xml`, the in-game floor numbering and the preview tabs, which now read `B1 1 2 B2 3`). `parameters.txt`: `levelOrder=1,2,B1,3`, written only when the order was actually changed, so a stock export is unaffected. Absent means the historical order and generates byte-identical output.
